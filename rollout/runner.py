"""
single-episode runners: the zero-coordination thinker+worker baseline, and the true single-model
baseline (no thinker at all).

this project previously included several coordinator variants (builds 04-09's fixed/heuristic
coordinators -- replan, mask, reward-aware, backtrack, verifier-triggered -- and build 13/14's
learned PPO coordinator). the coordinator line was removed from the codebase (kept only as
historical write-ups in reports/04-09 and results.md) after the learned coordinator's PPO training
runs converged on a real, precisely-diagnosed limitation rather than a usable result: the
verifier's per-turn signal (validated at the episode/aggregate level, see the verifier build
history) could not train a policy to discriminate its own actions' quality at the same state,
across every reward-construction and verifier variant tried. the paper's methodology centers on
the verifier itself (as a failure detector and via Best-of-N action selection, see
scripts/best_of_n_eval.py) rather than the coordinator, so this file now only contains what's
still load-bearing: the zero-coordination baseline every other build/report was measured against.
"""
